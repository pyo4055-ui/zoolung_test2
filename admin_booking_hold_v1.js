...
function save(list){if(typeof window.setStore==='function')window.setStore(KEY,list);else if(typeof setStore==='function')setStore(KEY,list);else localStorage.setItem(KEY,JSON.stringify(list))}
function msg(s){try{window.toast?.(s)}catch{}}
function complete(b){return !!(b?.status==='confirmed'&&b?.settlement?.savedAt)}
function adminOk(){try{if(typeof window.adminGuard==='function')return !!window.adminGuard(false);if(typeof adminGuard==='function')return !!adminGuard(false)}catch{}return !!window.zrReservationFirebase?.isStaff?.()}
...
function existingStatus(id,status){const b=byId(id);if(!b||b.status!==HOLD)return;const fn=typeof window.requestBookingStatus==='function'?window.requestBookingStatus:(typeof requestBookingStatus==='function'?requestBookingStatus:null);if(typeof fn!=='function')return msg('기존 예약 상태 변경 기능을 불러오지 못했습니다.');fn(id,status)}
...
function decorateActions(root,b,margin='14px'){
  if(!root||!b)return;
  const badge=root.querySelector('.row .status');if(b.status===HOLD&&badge){badge.textContent='예약보류';badge.className='status zr-hold-status'}
  if(root.querySelector(':scope > .zr-cal-state-row'))return;
...
function decorateActivityHoldBadges(){const root=$('activityList');if(!root)return;const hs=full().filter(b=>b.status===HOLD);root.querySelectorAll(':scope > .booking-item').forEach(card=>{const b=match(card,hs);if(!b)return;const badge=card.querySelector('.zr4-badges .status:not(.zr4-vendor),.row .status');if(badge){badge.textContent='예약보류';badge.className='status zr-hold-status'}})}
...
function renderHeld(){const root=$('activityList');if(!root)return;const list=heldActivity(),set=(id,n)=>{const e=$(id);if(e)e.textContent=`${n}건`};set('activityKpiTotal',list.length);set('activityKpiConfirmed',0);set('activityKpiPending',0);set('activityKpiCancelled',0);set('activityKpiCompleted',0);root.innerHTML=list.length?list.map(b=>`<div class="booking-item"><div class="zr4-badges"><span class="status zr-hold-status">예약보류</span></div><div class="row" style="margin-top:7px"><div><b>${esc(b.orgName)}</b><div class="help">접수 ${esc(seoulDate(b.createdAt)||'-')} · 예약일 ${esc(b.date||'-')}</div></div></div><div class="detail-grid"><div><b>예약자</b><br>${esc(b.managerName||'-')}</div><div><b>연락처</b><br>${esc(b.contact||'-')}</div><div><b>방문시간</b><br>${esc(b.entryTime||'--:--')} ~ ${esc(b.exitTime||'--:--')}</div><div><b>인원</b><br>유료 ${Number(b.paidCount||0)} / 인솔 ${Number(b.chaperoneCount||0)}</div></div><div style="display:flex;justify-content:flex-end;margin-top:10px"><button class="btn-soft" onclick="openAdminBookingDetail('${esc(b.id)}')">자세히</button></div></div>`).join(''):'<div class="help">선택한 조회 조건에 보류 예약이 없습니다.</div>'}

function wrap(){
  const active=typeof window.activeBookings==='function'?window.activeBookings:(typeof activeBookings==='function'?activeBookings:null);if(typeof active==='function'&&active!==activeWrap&&!active.__zrHold){const base=active,w=function(){const out=base.apply(this,arguments),r=Array.isArray(out)?[...out]:[],seen=new Set(r.map(x=>String(x?.id||'')));all().filter(b=>b.status===HOLD).forEach(b=>{const id=String(b.id||'');if(id&&!seen.has(id)){r.push(b);seen.add(id)}});return r};w.__zrHold=true;window.activeBookings=w;try{activeBookings=w}catch{}activeWrap=w}
  const badge=window.adminStatusBadge;if(typeof badge==='function'&&badge!==badgeWrap&&!badge.__zrHold){const base=badge,w=function(v){return v===HOLD?'<span class="status zr-hold-status">예약보류</span>':base.apply(this,arguments)};w.__zrHold=true;window.adminStatusBadge=w;try{adminStatusBadge=w}catch{}badgeWrap=w}
  ...
  const activity=window.renderActivity;if(typeof activity==='function'&&activity!==activityWrap&&!activity.__zrHold){const base=activity,w=function(){if(filterSelect()?.value===HOLD){renderHeld();return}const out=base.apply(this,arguments);decorateActivityHoldBadges();return out};w.__zrHold=true;window.renderActivity=w;try{renderActivity=w}catch{}activityWrap=w}
}
