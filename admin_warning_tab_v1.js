...
let installed=false,filter='all',pollTimer=null,lastSig='';
...
function inPrepWindow(b,now=today()){
  const diff=dayDiff(now,b.date);
  return diff!==null&&diff>=0&&diff<=PREP_DAYS;
}
...
function issuesFor(b,now=today()){
  ...
  if(diff>=0&&diff<=PREP_DAYS){
    ...
    if(!viewed(b,'customerViewedParkingAt'))out.push(issue('customer','주차안내 미확인','고객이 주차 및 인솔 안내를 아직 열람하지 않았습니다.',diff<=1?'danger':'info'));
    if(!viewed(b,'customerViewedGuideMapAt'))out.push(issue('customer','가이드맵 미확인','고객이 가이드맵을 아직 열람하지 않았습니다.',diff<=1?'danger':'info'));
    if(b.schedulePublished&&!viewed(b,'customerViewedScheduleAt'))out.push(issue('customer','최종 스케줄 미확인','확정된 관람 및 체험일정을 고객이 아직 열람하지 않았습니다.',diff<=1?'danger':'warn'));
  }
  return out;
}
function rows(){
...
function signature(){
  return JSON.stringify(confirmedBookings().map(b=>[
    ...
    b.schedulePublished,b.customerViewedParkingAt,b.customerViewedGuideMapAt,b.customerViewedScheduleAt,b.settlement?.savedAt,b.settlementCompletedAt
  ]));
}
...
function cardHtml(row){
  ...
  const paid=Number(b.paidCount||0),chap=Number(b.chaperoneCount||0);
  return `<article class="zr-warning-card" data-booking="${esc(b.id)}">
    ...
    <div class="zr-warning-actions"><button type="button" class="btn-gray" data-zr-warning-detail="${esc(b.id)}">예약 자세히</button></div>
  </article>`;
}
function counts(list){
  ...
  return {
    ...
    customer:allIssues.filter(i=>i.type==='customer').length,
...
function render(){
  ...
  if($('zrWarningCustomer'))$('zrWarningCustomer').textContent=`${c.customer}건`;
  if($('zrWarningSettlement'))$('zrWarningSettlement').textContent=`${c.settlement}건`;
  const root=$('zrWarningList');if(!root)return;
  root.innerHTML=shown.length?shown.map(cardHtml).join(''):`<div class="zr-warning-empty"><b>${filter==='all'?'현재 확인 필요한 경고가 없습니다.':'선택한 항목의 경고가 없습니다.'}</b><span>확정 예약의 기존 데이터만 계산해서 표시합니다.</span></div>`;
...
function ensureUi(){
  ...
  if(!sec){
    ...
    sec.innerHTML=`<div class="zr-warning-head"><div><h2>경고</h2><div class="zr-warning-help">방문 5일 전부터 준비 항목을 확인하고, 방문이 지난 확정 예약은 실제결제 누락을 표시합니다.<br>별도 경고 데이터는 저장하지 않습니다.</div></div><button type="button" class="btn-soft" id="zrWarningRefresh">새로고침</button></div>
      <div class="zr-warning-summary">
        ...
        <div class="zr-warning-metric"><label>고객 미확인</label><strong id="zrWarningCustomer">0건</strong></div>
      ...
      <div class="zr-warning-filterbox"><b>경고 종류</b><div class="zr-warning-filters" id="zrWarningFilters">
        ...
        <button type="button" class="btn-gray" data-zr-warning-filter="customer">고객 미확인</button>
