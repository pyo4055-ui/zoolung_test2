...
const addTime=(start,duration)=>{
  const a=minutes(start),d=Number(duration||0);
  if(a===null||!Number.isFinite(d)||d<=0)return '';
  const n=a+d;return `${pad(Math.floor(n/60))}:${pad(n%60)}`;
};
const FIXED_NAMES={f4:'4F 베이직',f5:'5F 워터가든',meal:'식사',play:'놀이터'};
...
function datedBookings(){
  return allBookings().filter(b=>String(b?.date||'')&&String(b?.status||'')!=='cancelled');
}
function settlementDone(b){return !!String(b?.settlement?.savedAt||b?.settlementCompletedAt||'').trim()}
function notified(b){return !!String(b?.customerSchedule?.notifiedAt||'').trim()}
function issue(type,label,detail,severity='warn'){return {type,label,detail,severity}}
function segmentName(s){return String(s?.label||FIXED_NAMES[s?.type]||s?.type||'컨텐츠')}
function currentMealSegment(b){
  if(String(b?.mealType||'none')==='none'||!String(b?.mealStart||'').trim())return null;
  const start=String(b.mealStart),end=String(b.mealEnd||addTime(start,b.mealDuration));
  return end?{type:'meal',label:'식사',start,end}:null;
}
function currentPlaySegment(b){
  if(String(b?.playUse||'no')==='no'||!String(b?.playStart||'').trim())return null;
  const start=String(b.playStart),end=String(b.playEnd||addTime(start,b.playDuration));
  return end?{type:'play',label:'놀이터',start,end}:null;
}
function effectiveScheduleSegments(b){
  const source=Array.isArray(b?.customerSchedule?.segments)?b.customerSchedule.segments:[];
  const out=source.filter(s=>s&&s.type!=='meal'&&s.type!=='play').map(s=>({...s}));
  const meal=currentMealSegment(b),play=currentPlaySegment(b);
  if(meal)out.push(meal);if(play)out.push(play);
  return out;
}
function scheduleConflicts(b){
  if(!b?.schedulePublished||!Array.isArray(b?.customerSchedule?.segments))return [];
  const active=effectiveScheduleSegments(b).map(s=>({...s,a:minutes(s.start),z:minutes(s.end)}))
    .filter(s=>s.a!==null&&s.z!==null&&s.z>s.a)
    .sort((x,y)=>x.a-y.a||x.z-y.z);
  const found=[];
  for(let i=0;i<active.length;i++){
    for(let j=i+1;j<active.length;j++){
      const a=active[i],b2=active[j];
      if(b2.a>=a.z)break;
      const overlap=Math.min(a.z,b2.z)-Math.max(a.a,b2.a);
      if(overlap>0)found.push({a,b:b2,overlap});
    }
  }
  return found;
}
function issuesFor(b,now=today()){
  const diff=dayDiff(now,b.date);if(diff===null)return [];
  const out=[],status=String(b?.status||'');

  if(status==='confirmed'&&diff<=-PREP_DAYS&&!settlementDone(b)){
  ...
  if(diff>=0&&diff<=PREP_DAYS){
    if(['pending','hold'].includes(status)){
      out.push(issue('booking','예약 미확정',diff===0?'오늘 방문 예정이지만 예약이 아직 확정되지 않았습니다.':`방문 ${diff}일 전 · 예약이 아직 확정되지 않았습니다.`,diff<=1?'danger':'warn'));
      return out;
    }
    if(status!=='confirmed')return out;

    if(!b.schedulePublished){
      out.push(issue('schedule','스케줄 미확정',diff===0?'오늘 방문인데 최종 스케줄이 확정되지 않았습니다.':`방문 ${diff}일 전 · 최종 스케줄이 확정되지 않았습니다.`,diff<=1?'danger':'warn'));
    }else{
      if(!notified(b))out.push(issue('notify','고객 알림 미완료',diff===0?'오늘 방문인데 스케줄 안내 문자를 아직 준비하지 않았습니다.':`방문 ${diff}일 전 · 확정 스케줄 고객 알림이 아직 완료되지 않았습니다.`,diff<=1?'danger':'warn'));
      scheduleConflicts(b).forEach(x=>{
        out.push(issue('conflict','스케줄 시간 겹침',`${segmentName(x.a)} ${x.a.start}~${x.a.end} ↔ ${segmentName(x.b)} ${x.b.start}~${x.b.end} · ${x.overlap}분 겹칩니다.`,diff<=1?'danger':'warn'));
      });
    }
  }
  return out;
...
function rows(){
  const now=today();
  return datedBookings().map(b=>({booking:b,issues:issuesFor(b,now)})).filter(x=>x.issues.length)
...
function signature(){
  return JSON.stringify(datedBookings().map(b=>[
    b.id,b.status,b.date,b.entryTime,b.orgName,b.mealType,b.mealStart,b.mealEnd,b.mealDuration,b.playUse,b.playStart,b.playEnd,b.playDuration,
    b.schedulePublished,b.customerSchedule?.notifiedAt,b.customerSchedule?.segments,b.settlement?.savedAt,b.settlementCompletedAt
  ]));
}
...
function relativeLabel(v){
  const diff=dayDiff(today(),v);if(diff===null)return '';
  if(diff===0)return '오늘';if(diff===1)return '내일';if(diff>1)return `D-${diff}`;return `D+${Math.abs(diff)}`;
}
function severityRank(s){return s==='danger'?0:s==='warn'?1:2}
function issueHtml(i){return `<div class="zr-warning-issue ${esc(i.severity)}"><div><b>${esc(i.label)}</b><span>${esc(i.detail)}</span></div></div>`}
...
function counts(list){
  ...
  return {
    ...
    booking:allIssues.filter(i=>i.type==='booking').length,
    schedule:allIssues.filter(i=>i.type==='schedule'||i.type==='conflict').length,
...
function render(){
  ...
  if($('zrWarningBooking'))$('zrWarningBooking').textContent=`${c.booking}건`;
  ...
  root.innerHTML=pageRows.length?pageRows.map(cardHtml).join(''):`<div class="zr-warning-empty"><b>${filter==='all'?'현재 확인 필요한 경고가 없습니다.':'선택한 항목의 경고가 없습니다.'}</b><span>기존 예약·스케줄 데이터만 계산해서 표시합니다.</span></div>`;
...
function setFilter(next){filter=String(next||'all');page=1;render()}
function setPage(next){
  const shown=filteredRows(rows()),totalPages=Math.max(1,Math.ceil(shown.length/PAGE_SIZE));
  if(next==='prev')page=Math.max(1,page-1);else if(next==='next')page=Math.min(totalPages,page+1);else page=Math.min(totalPages,Math.max(1,Number(next)||1));
  render();$('tab-warning')?.scrollIntoView?.({block:'start',behavior:'smooth'});
}
...
function openTab(){
  ...
  $('tab-warning')?.classList.remove('hidden');if($('zrWarningTabBtn'))$('zrWarningTabBtn').className='btn-primary';render();startPoll();
}
...
.zr-warning-actions{display:flex;flex-direction:column;align-items:stretch;justify-content:center;gap:7px;padding:12px;border-left:1px solid #edf0ed}.zr-warning-actions button{width:100%;margin:0;white-space:nowrap}.zr-warning-empty{padding:36px 12px;border:1px dashed var(--zw-line);border-radius:14px;background:#fff;text-align:center;color:var(--zw-muted)}.zr-warning-empty b,.zr-warning-empty span{display:block}.zr-warning-empty span{margin-top:5px;font-size:11px}
.zr-warning-pagination{display:flex;align-items:center;justify-content:center;gap:10px;flex-wrap:wrap;margin:14px 0 4px}.zr-warning-page-info{font-size:11px;color:var(--zw-muted);font-weight:800}.zr-warning-page-buttons{display:flex;gap:5px;flex-wrap:wrap;justify-content:center}.zr-warning-page-buttons button{margin:0;min-width:36px;padding:7px 9px}.zr-warning-page-buttons button:disabled{opacity:.45;cursor:default}
...
function ensureUi(){
  ...
  const tabs=document.querySelector('#adminView .admin-tabs'),admin=$('adminView');if(!tabs||!admin)return false;
  installStyle();
  let btn=$('zrWarningTabBtn');if(!btn){btn=document.createElement('button');btn.id='zrWarningTabBtn';btn.className='btn-gray';btn.textContent='경고';const todayBtn=$('zrTodayTabBtn');tabs.insertBefore(btn,todayBtn?.nextSibling||tabs.firstChild)}
  let sec=$('tab-warning');
  if(!sec){
    sec=document.createElement('section');sec.id='tab-warning';sec.className='hidden';
    sec.innerHTML=`<div class="zr-warning-head"><div><h2>경고</h2><div class="zr-warning-help">방문 D-5부터 예약 미확정·스케줄 미확정·고객 알림 미완료·확정 스케줄 시간 겹침을 확인하고, 실제결제 미입력은 방문 D+5부터 표시합니다.<br>별도 경고 데이터는 저장하지 않습니다.</div></div><button type="button" class="btn-soft" id="zrWarningRefresh">새로고침</button></div>
      <div class="zr-warning-summary">
        ...
        <div class="zr-warning-metric"><label>예약 미확정</label><strong id="zrWarningBooking">0건</strong></div>
        <div class="zr-warning-metric"><label>스케줄 문제</label><strong id="zrWarningSchedule">0건</strong></div>
      ...
      <div class="zr-warning-filterbox"><b>경고 종류</b><div class="zr-warning-filters" id="zrWarningFilters">
        ...
        <button type="button" class="btn-gray" data-zr-warning-filter="booking">예약 미확정</button>
        <button type="button" class="btn-gray" data-zr-warning-filter="schedule">스케줄 미확정</button>
        <button type="button" class="btn-gray" data-zr-warning-filter="notify">고객 알림</button>
        <button type="button" class="btn-gray" data-zr-warning-filter="conflict">시간 겹침</button>
        <button type="button" class="btn-gray" data-zr-warning-filter="settlement">정산/실결제</button>
      </div></div>
      <div class="zr-warning-list" id="zrWarningList"></div><div class="zr-warning-pagination" id="zrWarningPagination"></div>`;
    admin.appendChild(sec);
  }
  btn.onclick=openTab;
  $('zrWarningRefresh').onclick=render;
  ...
  $('zrWarningList').onclick=e=>{
    const settle=e.target?.closest?.('[data-zr-warning-settlement]');if(settle){openSettlement(settle.dataset.zrWarningSettlement);return}
    const detail=e.target?.closest?.('[data-zr-warning-detail]');if(!detail)return;
    const id=detail.dataset.zrWarningDetail;if(typeof window.openAdminBookingDetail==='function')window.openAdminBookingDetail(id);else if(typeof globalThis.openAdminBookingDetail==='function')globalThis.openAdminBookingDetail(id);
  };
  installed=true;render();return true;
}
function boot(){if(ensureUi())return;let tries=0;const timer=setInterval(()=>{if(ensureUi()||++tries>40)clearInterval(timer)},250)}
