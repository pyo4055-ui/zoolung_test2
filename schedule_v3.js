...
{id:id(),date:d,org:"위즈아이어린이집",res:24,tea:5,actualPaid:"",actualChap:"",ps:"10:30",pe:"14:00",eta:"",aend:"",meal:"도시락",mealLoc:"",cafeDetail:"",pay:false,book:false,memo:"",cancelled:false,segments:segs(["f4","meal","play","f5"])},
{id:id(),date:d,org:"비봉어린이집",res:11,tea:4,actualPaid:"",actualChap:"",ps:"10:30",pe:"14:30",eta:"",aend:"",meal:"카페주문",mealLoc:"5F 카페",cafeDetail:"아메리카노 3\n뽀로로음료 11",pay:false,book:false,memo:"",cancelled:false,segments:segs(["f4","meal","f5","play"])},
{id:id(),date:d,org:"아이해피어린이집",res:18,tea:3,actualPaid:"",actualChap:"",ps:"10:45",pe:"14:15",eta:"",aend:"",meal:"도시락",mealLoc:"",cafeDetail:"",pay:false,book:false,memo:"만 1~2세",cancelled:false,segments:segs(["f5","meal","play","f4"])},
{id:id(),date:d,org:"피오리어린이집",res:21,tea:4,actualPaid:"",actualChap:"",ps:"11:00",pe:"14:30",eta:"",aend:"",meal:"식사없음",mealLoc:"",cafeDetail:"",pay:false,book:false,memo:"개구쟁이어린이집 연합",cancelled:false,segments:segs(["play","f4","free","f5"])},
{id:id(),date:d,org:"주렁어린이집",res:30,tea:6,actualPaid:"",actualChap:"",ps:"10:30",pe:"14:30",eta:"",aend:"",meal:"카페주문",mealLoc:"5F 카페",cafeDetail:"어린이음료 20\n아메리카노 4",pay:false,book:false,memo:"",cancelled:false,segments:segs(["f5","play","meal","f4"])}
]}

let data=get(KEY,null);if(!data||!Array.isArray(data.groups)){data={groups:samples(today())};set(KEY,data)}
function normalize(){
...
normalize();
let date=get(PK,today()),editMode=false,eid=null,draft=null,contentState=null;
...
function render(){
 ...
 $("list").innerHTML=a.map(g=>{
   ...
   <div class="head"><div class="org">${esc(g.org)}</div><span class="tag">${g.res}(${g.tea})명</span><span class="${mealClass}" ${cafe?`data-cafe="${g.id}"`:""}>${esc(g.meal||"")}</span>${g.mealLoc?`<span class="tag locTag">${esc(g.mealLoc)}</span>`:""}${g.cancelled?'<span class="tag cancelTag">당일취소</span>':""}<button class="soft detail" data-e="${g.id}">상세</button></div>
...
function fresh(){return {id:id(),date,org:"",res:15,tea:0,actualPaid:"",actualChap:"",ps:"10:30",pe:"14:00",eta:"",aend:"",meal:"도시락",mealLoc:"",cafeDetail:"",pay:false,book:false,memo:"",cancelled:false,segments:segs(["f4","meal","play","f5"])}}
function openDetail(i){if(!editMode)return lockedMsg();eid=i||null;draft=JSON.parse(JSON.stringify(i?data.groups.find(g=>g.id===i):fresh()));$("detailTitle").textContent=i?"단체 상세 수정":"단체 스케줄 추가";["org","res","tea","actualPaid","actualChap","ps","pe","eta","aend","meal","mealLoc","cafeDetail","memo"].forEach(k=>$(k).value=draft[k]??"");refreshDetailChecks();refreshCancelBtn();$("detailModal").classList.remove("hidden")}
...
$("detailSave").onclick=()=>{["org","res","tea","actualPaid","actualChap","ps","pe","eta","aend","meal","mealLoc","cafeDetail","memo"].forEach(k=>draft[k]=$(k).value);draft.res=Number(draft.res||0);draft.tea=Number(draft.tea||0);draft.actualPaid=draft.actualPaid===""?"":Number(draft.actualPaid||0);draft.actualChap=draft.actualChap===""?"":Number(draft.actualChap||0);if(!draft.org)return toast("단체명을 입력해주세요.");if(min(draft.pe)<=min(draft.ps))return toast("퇴장시간을 도착시간보다 늦게 설정해주세요.");if(eid)data.groups[data.groups.findIndex(g=>g.id===eid)]=draft;else data.groups.push(draft);set(KEY,data);$("detailModal").classList.add("hidden");render();toast("저장했습니다.")};
...
function openCafe(gid){let g=data.groups.find(z=>z.id===gid);if(!g)return;$("cafeTitle").textContent=g.org+" · 카페 주문";$("cafeBody").textContent=(g.cafeDetail||"").trim()||"등록된 카페 메뉴 정보가 없습니다.";$("cafeModal").classList.remove("hidden")}
$("cafeClose").onclick=()=>$("cafeModal").classList.add("hidden");$("cafeModal").onclick=e=>{if(e.target===$("cafeModal"))$("cafeModal").classList.add("hidden")};

$("add").onclick=$("fab").onclick=()=>openDetail();
