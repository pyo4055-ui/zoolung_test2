...
function paintArrow(btn,hover=false){
  ...
  const set=(k,v)=>{if(btn.style.getPropertyValue(k)!==v||btn.style.getPropertyPriority(k)!=='important')btn.style.setProperty(k,v,'important')};
  const size=window.matchMedia('(max-width:900px)').matches?'44px':'32px';
  set('width',size);set('height',size);set('min-width',size);set('min-height',size);set('padding','0');
...
function decorateButton(btn){
  if(!btn)return;
  // Popups and the shared memo own their controls. Do not attach inline
  // wine-color hover/focus handlers to controls that can move into a dialog.
  if(btn.closest('#zrAdminDailyMemoV1,.modal,.zr14-modal,[role="dialog"]'))return;
...
function bind(){
  ...
  window.matchMedia('(max-width:900px)').addEventListener('change',()=>decorateArrows($('adminView')||document));
