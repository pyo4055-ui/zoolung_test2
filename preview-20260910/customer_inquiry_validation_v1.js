...
function collectIssues(){
  const out=[];
  const type=$('inqType'),year=$('inqVisitYear'),month=$('inqVisitMonth'),day=$('inqVisitDay'),time=$('inqVisitTime'),people=$('inqPeople');
  ...
  if((year&&!norm(year.value))||!norm(month?.value)||!norm(day?.value))out.push(issue(dateTarget(),year&&!norm(year.value)?year:!norm(month?.value)?month:day,'방문 희망일을 선택해주세요.','date'));
...
function clearEdited(e){
  ...
  const ids=['inqType','inqVisitYear','inqVisitMonth','inqVisitDay','inqVisitTime','inqPeople','inqOrgName','inqName','inqMobile','inqContent','inqPrivacy','zrChangeBookingSelect'];
  if(!ids.includes(e.target.id))return;
  if(['inqVisitYear','inqVisitMonth','inqVisitDay'].includes(e.target.id))clearOne(dateTarget());
