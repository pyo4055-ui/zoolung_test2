...
let observer=null,bootBound=false,decorationQueued=false;

function ensureSafariTheme(){
  ['zrAdminSafariThemeV1','zrAdminSafariThemeV2'].forEach(id=>$(id)?.remove());
  if(!$('zrAdminSafariThemeV3')){
    const link=document.createElement('link');
    link.id='zrAdminSafariThemeV3';link.rel='stylesheet';link.href='./admin_safari_theme_v3.css?v=5';
...
function injectStyle(){
  ...
  const s=document.createElement('style');s.id='zrAdminShellCurrentUiFixV1Style';s.textContent=`
    /* Shared popup header owns dismissal; preserve existing close handlers.
       Login is a full-page entry and keeps its separate visual treatment. */
    html.zr-admin-entry-page .modal:not(#adminLoginModal) .zr-modal-ux-header,
    html.zr-admin-entry-page .zr-admin-help-head{
      position:sticky!important;top:0!important;z-index:50!important;
      background:var(--zr-v3-orange,#fc5404)!important;color:#fff!important;
      border-bottom:0!important;
    }
    html.zr-admin-entry-page .modal:not(#adminLoginModal) .zr-modal-ux-header-title,
    html.zr-admin-entry-page .zr-admin-help-head strong{
      color:#fff!important;-webkit-text-fill-color:#fff!important;
    }
    html.zr-admin-entry-page .modal:not(#adminLoginModal) .zr-modal-ux-header-close,
    html.zr-admin-entry-page .zr-admin-help-close{
      background:#fff!important;color:var(--zr-v3-brown,#470910)!important;
      -webkit-text-fill-color:var(--zr-v3-brown,#470910)!important;
      border:1px solid #eaded5!important;min-width:44px!important;min-height:44px!important;
    }
    html.zr-admin-entry-page .modal:not(#adminLoginModal) .modal-actions{
      background:#fff!important;
    }
    html.zr-admin-entry-page .modal:not(#adminLoginModal) .zr-modal-ux-header-close:focus-visible,
    html.zr-admin-entry-page .zr-admin-help-close:focus-visible{
      outline:2px solid #470910!important;outline-offset:2px!important;
    }
    ...
    /* PC role accents: browsing a detail is distinct from completing work. */
    @media(min-width:901px){
      html.zr-admin-shell-mounted body #adminView button.zr-safari-popup-trigger{
        background:var(--zr-v3-brown)!important;border-color:var(--zr-v3-brown)!important;
        color:#fff!important;-webkit-text-fill-color:#fff!important;box-shadow:none!important;
      }
      html.zr-admin-shell-mounted body #adminView button.zr-safari-popup-trigger:hover{
        background:var(--zr-v3-brown-dark)!important;border-color:var(--zr-v3-brown-dark)!important;
      }
      html.zr-admin-shell-mounted body #adminView button.zr-safari-role-complete,
      html.zr-admin-entry-page body button[data-zr-cancel-review],
      html.zr-admin-shell-mounted body #adminView #zrCancelReviewTodayV1{
        background:var(--zr-v3-green)!important;border-color:var(--zr-v3-green)!important;
        color:#fff!important;-webkit-text-fill-color:#fff!important;box-shadow:none!important;
      }
      html.zr-admin-shell-mounted body #adminView button.zr-safari-role-complete:hover,
      html.zr-admin-entry-page body button[data-zr-cancel-review]:hover,
      html.zr-admin-shell-mounted body #adminView #zrCancelReviewTodayV1:hover{
        background:var(--zr-v3-green-dark)!important;border-color:var(--zr-v3-green-dark)!important;
      }
      html.zr-admin-shell-mounted body #adminView #zrCancelReviewToolbarV1 :is(input,select,button),
      html.zr-admin-shell-mounted body #adminView .zr-cancel-workspace-actions button{
        box-sizing:border-box!important;min-height:42px!important;height:42px!important;
      }
      html.zr-admin-shell-mounted body #adminView .zr-cancel-workspace-actions{align-items:center;flex-wrap:wrap;gap:8px}
    }
...
function decorateBrand(){
  ...
  if(title&&!title.querySelector('.zr-admin-brand-dongtan')){title.dataset.zrSafariBrand='1';title.innerHTML='<span class="zr-admin-brand-dongtan">동탄점</span>'}
  if(sub&&sub.textContent!=='예약관리')sub.textContent='예약관리';
}
...
function decorateActionButtons(){
  ...
  root.querySelectorAll('button').forEach(btn=>{
    ...
    // Navigation labels must not be classified as popup actions by their text.
    if(btn.closest('.zr-admin-subtabs,#zrCleanupInnerTabs,#zrInquiryReplyInnerTabs')){
      btn.classList.remove('zr-safari-popup-trigger','zr-safari-payment-trigger','zr-safari-role-complete',...roleClasses);
      return;
    }
    btn.classList.toggle('zr-safari-role-complete',['확인완료','처리완료'].includes(compact));
    const inCalendarDay=!!btn.closest('#adminCalendar .day');
    if(inCalendarDay&&text==='자세히'){
      btn.classList.remove('zr-safari-popup-trigger','zr-safari-payment-trigger',...roleClasses);
      return;
    }
    const payment=text==='실제결제'||btn.classList.contains('zr-settle-open');
    const detail=text==='자세히'||text==='예약 자세히'||text==='상세보기'||text==='상세'||/openAdminBookingDetail\s*\(/.test(onclick);
    ...
    btn.classList.toggle('zr-safari-popup-trigger',!payment&&popup);
...
function boot(){
  if(bootBound){apply();return}
  bootBound=true;
  apply();let tries=0;const wait=setInterval(()=>{apply();if(($('zrAdminShellRefresh')&&$('adminView'))||++tries>120)clearInterval(wait)},100);
  const admin=$('adminView');if(admin&&!observer){observer=new MutationObserver(()=>{
    if(decorationQueued)return;
    decorationQueued=true;
    requestAnimationFrame(()=>{decorationQueued=false;hideLegacyChrome();decorateBrand();decorateActionButtons();decorateCalendar()});
  });observer.observe(admin,{subtree:true,childList:true})}
