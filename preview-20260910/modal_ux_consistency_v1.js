...
function syncHeaderGeometry(shell,header){
  try{
    ...
    if(header.style.getPropertyValue('--zr-modal-ux-shell-pad-left')!==`${pl}px`)header.style.setProperty('--zr-modal-ux-shell-pad-left',`${pl}px`);
    if(header.style.getPropertyValue('--zr-modal-ux-shell-pad-right')!==`${pr}px`)header.style.setProperty('--zr-modal-ux-shell-pad-right',`${pr}px`);
  }catch{}
}
function syncOverlayHeader(overlay){
  ...
  if(titleSource){
    ...
    if(titleEl.textContent!==textOf(titleSource))titleEl.textContent=textOf(titleSource);
  ...
  if(closeSource){
    ...
    const closeText=icon?'✕':'닫기';
    if(closeEl.textContent!==closeText)closeEl.textContent=closeText;
