...
function installStyle(){
  ...
  style.textContent=`
    ...
    #zrInquiryVisitFields .zr-inquiry-date-selects{display:grid;grid-template-columns:minmax(0,1.1fr) minmax(0,1fr) minmax(0,1fr);gap:8px;width:100%}
    #zrInquiryVisitFields select{width:100%;max-width:100%;min-width:0;box-sizing:border-box;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:14px}
    #zrInquiryVisitFields .zr-inquiry-people{width:100%;margin-bottom:0}
    #zrInquiryVisitFields #inqVisitTimeHelp{display:none!important}
    ...
    @media(max-width:800px){
      ...
      #zrInquiryVisitFields .zr-inquiry-visit-grid>div:first-child{grid-column:1/-1}
      #zrInquiryVisitFields .zr-inquiry-visit-grid>div:nth-child(2){grid-column:1/-1}
      #zrInquiryVisitFields .zr-inquiry-date-selects{grid-template-columns:minmax(0,1.15fr) minmax(0,1fr) minmax(0,1fr);gap:6px;min-width:0}
      #zrInquiryVisitFields select{min-width:0;padding-left:8px;padding-right:8px;font-size:13px}
...
function install(){
      ...
      <div class="zr-inquiry-visit-grid">
        <div>
          <label class="req">방문 희망일</label>
            ...
            <select id="inqVisitYear"><option value="">년도 선택</option></select>
            <select id="inqVisitMonth" disabled><option value="">월 선택</option></select>
        ...
        <div>
          ...
          <select id="inqVisitTime" disabled><option value="">시간 선택</option></select>
          <div class="help" id="inqVisitTimeHelp"></div>
        </div>
      </div>
  ...
  const visitYear=document.getElementById('inqVisitYear');
  ...
  if(!type||!visitYear||!visitMonth||!visitDay||!visitDate||!visitTime||!visitTimeHelp||!people||!peopleLabel||!org||!name||!mobile||!email||!privacy)return false;
  ...
  function invalidNode(el){
    ...
    if(el===visitYear||el===visitMonth||el===visitDay)return el.closest('.zr-inquiry-date-selects')||el;
    return el;
  }
  ...
  function fillYears(){
    const current=visitYear.value,today=new Date(),start=today.getFullYear();
    const options=['<option value="">년도 선택</option>'];
    for(let year=start;year<=start+10;year++)options.push(`<option value="${year}">${year}년</option>`);
    visitYear.innerHTML=options.join('');
    if([...visitYear.options].some(o=>o.value===current))visitYear.value=current;
  }
  function fillMonths(){
    const current=visitMonth.value,year=Number(visitYear.value);
    if(!year){visitMonth.innerHTML='<option value="">월 선택</option>';visitMonth.disabled=true;return}
    const options=['<option value="">월 선택</option>'];
    for(let month=1;month<=12;month++)options.push(`<option value="${pad2(month)}">${month}월</option>`);
    visitMonth.innerHTML=options.join('');visitMonth.disabled=false;
    if([...visitMonth.options].some(o=>o.value===current))visitMonth.value=current;
  }
  function fillDays(){
    const current=visitDay.value;
    const year=Number(visitYear.value),mon=Number(visitMonth.value);
    if(!year||!mon){visitDay.innerHTML='<option value="">일 선택</option>';visitDay.disabled=true;visitDate.value='';return}
    const month=`${year}-${pad2(mon)}`;
  ...
  function syncDate(){
    visitDate.value=visitYear.value&&visitMonth.value&&visitDay.value?`${visitYear.value}-${visitMonth.value}-${visitDay.value}`:'';
  }
  function fillTimeOptions(){
    const current=visitTime.value;
    const isGroup=type.value==='group',isPreview=type.value==='preview';
    if(!isGroup&&!isPreview){
      visitTime.innerHTML='<option value="">시간 선택</option>';
      visitTime.disabled=true;
      visitTimeHelp.textContent='';
      return;
    }
  ...
  function resetVisitFields(){
    clearInvalidMarks();type.value='';visitYear.value='';visitMonth.value='';visitDay.innerHTML='<option value="">일 선택</option>';visitDay.disabled=true;visitDate.value='';visitTime.value='';people.value='';org.value='';draft=null;fillYears();fillMonths();updateTypeUi();showStage(stages,'form');
  }
  ...
  function collectDraft(){
    ...
    if(!date)missing.push(!visitYear.value?visitYear:!visitMonth.value?visitMonth:visitDay);
  ...
  fillYears();fillMonths();fillDays();updateTypeUi();
  if(visitYear.dataset.zrBound!=='1'){visitYear.dataset.zrBound='1';visitYear.addEventListener('change',()=>{visitMonth.value='';visitDay.value='';fillMonths();fillDays()})}
