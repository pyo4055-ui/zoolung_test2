...
let FS=null,db=null,unsub=null,current=clone(DEF),guideDoc={},submitBypass=false,pendingButton=null;
...
async function init(){
  try{FS=await import(`https://www.gstatic.com/firebasejs/${FV}/firebase-firestore.js`);const z=window.zrReservationFirebase;if(!z?.db)throw new Error('Firebase DB bridge unavailable');db=z.db;if(unsub)unsub();unsub=FS.onSnapshot(FS.doc(db,COLLECTION,DOC_ID),snap=>{guideDoc=snap.exists()?snap.data()||{}:{};current=norm(guideDoc.parking);renderCard();if($('zrGuideAdminSection')&&!$('zrGuideAdminSection').classList.contains('hidden'))renderAdmin()},e=>console.error('parking v31 read',e))}catch(e){console.error('parking v31 init',e)}
}
function boot(){
  injectStyle();const cardTimer=setInterval(()=>{if(renderCard())clearInterval(cardTimer)},200);setTimeout(()=>clearInterval(cardTimer),20000);const adminTimer=setInterval(()=>{const t=$('zrGuideAdminTab');if(!t)return;t.addEventListener('click',()=>setTimeout(renderAdmin,80));clearInterval(adminTimer)},250);setTimeout(()=>clearInterval(adminTimer),20000);bindFinal();const wait=setInterval(()=>{if(!window.zrReservationFirebase?.auth||!window.zrReservationFirebase?.db)return;clearInterval(wait);init()},200);setTimeout(()=>clearInterval(wait),20000);
}
