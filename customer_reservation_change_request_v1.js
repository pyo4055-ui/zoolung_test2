...
function fillDays(preferred='',preserve=false){const month=String($('zrChangeVisitMonth')?.value||''),sel=$('zrChangeVisitDay');if(!sel)return;const current=preserve?String(sel.value||''):preferred;if(!/^\d{4}-\d{2}$/.test(month)){sel.innerHTML='<option value="">일 선택</option>';sel.disabled=true;return}const [year,mon]=month.split('-').map(Number),last=new Date(year,mon,0).getDate(),today=localToday();const native=$('visitDay'),nativeRows=native?.tagName==='SELECT'?[...native.options].filter(o=>DATE_RE.test(String(o.value||''))&&String(o.value).startsWith(month+'-')):[];const rows=nativeRows.length?nativeRows.map(o=>({day:String(o.value).slice(8,10),text:String(o.textContent||`${Number(String(o.value).slice(8,10))}일`),disabled:!!o.disabled||String(o.value)<=today})):(()=>{const out=[];for(let d=1;d<=last;d++){const dd=pad2(d),full=`${month}-${dd}`;out.push({day:dd,text:`${d}일`,disabled:full<=today})}return out})();sel.innerHTML='<option value="">일 선택</option>'+rows.map(x=>`<option value="${x.day}"${x.disabled?' disabled':''}>${esc(x.text)}</option>`).join('');sel.disabled=false;if(current&&[...sel.options].some(o=>o.value===current&&!o.disabled))sel.value=current;else sel.value=''}
