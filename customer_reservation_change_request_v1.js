...
function localTomorrow(){const d=new Date();d.setHours(12,0,0,0);d.setDate(d.getDate()+1);return `${d.getFullYear()}-${pad2(d.getMonth()+1)}-${pad2(d.getDate())}`}
function isWeekendDate(date){
  if(!DATE_RE.test(String(date||'')))return false;
  const [y,m,d]=String(date).split('-').map(Number),day=new Date(y,m-1,d,12).getDay();
  return day===0||day===6;
}
function holidayBookingAllowed(){
  try{
    const fn=typeof window.settings==='function'?window.settings:(typeof settings==='function'?settings:null);
    const s=fn?.();return s?.holidayBookingAllowed!==false;
  }catch{return true}
}
function isBlockedHoliday(date){
  if(holidayBookingAllowed())return false;
  try{return window.zrHolidayBookingSettingV1Api?.isHoliday?.(date)===true}catch{return false}
}
function changeDateUnavailable(date,nativeOption=null){
  const v=String(date||'');
  if(!DATE_RE.test(v)||v<localTomorrow())return true;
  if(isWeekendDate(v)||isBlockedHoliday(v))return true;
  return nativeOption?.disabled===true;
}
...
function rangeDuration(start,end,allowed,fallback){const s=timeToMin(start),e=timeToMin(end),d=e-s;return Number.isFinite(s)&&Number.isFinite(e)&&e>s&&allowed.includes(d)?d:fallback}
function optionTimeValues(id){const el=$(id);if(!el||el.tagName!=='SELECT')return [];return [...new Set([...el.options].map(o=>String(o.value||'').trim()).filter(v=>/^\d{2}:\d{2}$/.test(v)))].sort()}
function generatedTimes(start=10*60+30,end=18*60){const out=[];for(let m=start;m<=end;m+=30)out.push(minToTime(m));return out}
...
function toastSafe(msg){try{if(typeof window.toast==='function'){window.toast(msg);return}}catch{}try{alert(msg)}catch{}}
function clearInvalid(){$('zrReservationChangeForm')?.querySelectorAll('.zr-change-invalid').forEach(el=>el.classList.remove('zr-change-invalid'));$('zrChangeFormError')?.classList.add('hidden')}
function failForm(message,control){
  showStage('form');clearInvalid();const target=typeof control==='string'?$(control):control;
  ...
  toastSafe(message);setTimeout(()=>{try{target?.scrollIntoView?.({behavior:'smooth',block:'center'});target?.focus?.({preventScroll:true})}catch{}},40);return null;
}
...
function ensureStyle(){
  ...
  #zrReservationChangeSelectV1 .zr-change-select-sheet{width:min(620px,100%);max-height:min(92vh,860px);padding:0;overflow:auto;background:#fff;color:#35261f;border-radius:18px;box-shadow:0 24px 80px rgba(0,0,0,.26);-webkit-overflow-scrolling:touch}
  #zrReservationChangeSelectV1 .zr-change-select-head{position:sticky;top:0;z-index:3;display:flex!important;align-items:center;gap:10px;margin:0;padding:14px 18px;background:#fc5404;color:#fff;border-bottom:1px solid #e84d04}
  #zrReservationChangeSelectV1 .zr-change-select-head h2{margin:0;flex:1;font-size:20px;color:#fff}
  ...
  #zrReservationChangeSelectV1 .zr-change-select-help{margin:0;padding:16px 18px 12px;color:#6d5b52;font-size:13px;line-height:1.6}
  #zrReservationChangeSelectV1 .zr-change-select-list{display:grid;gap:10px;padding:0 18px 18px}
  ...
  #zrReservationChangeNoticeV1 .zr-change-notice-head{padding:18px 22px;background:#fc5404;color:#fff;border-bottom:1px solid #e84d04;font-size:20px;font-weight:950}
...
function ensureChangeSelect(){
  ...
  document.body.appendChild(modal);modal.querySelector('.zr-change-select-close').onclick=closeChangeSelect;
  modal.addEventListener('click',e=>{if(e.target===modal){closeChangeSelect();return}const btn=e.target?.closest?.('[data-zr-change-select]');if(!btn)return;const id=String(btn.dataset.zrChangeSelect||'');closeChangeSelect();openNotice(id)});return modal;
}
function openChangeSelect(){const rows=activeChangeBookings();if(!rows.length){toastSafe('변경할 수 있는 예약 내역이 없습니다.');return}const modal=ensureChangeSelect(),list=$('zrReservationChangeSelectList');const ordered=[...rows].sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))||String(a.entryTime||'').localeCompare(String(b.entryTime||'')));list.innerHTML=ordered.map(changeChoiceHtml).join('');modal.classList.remove('hidden')}
function closeChangeSelect(){$('zrReservationChangeSelectV1')?.classList.add('hidden')}
...
function ensureNotice(){
  ...
  document.body.appendChild(modal);$('zrReservationChangeNoticeCancel').onclick=()=>{pendingBookingId='';closeNotice()};$('zrReservationChangeNoticeConfirm').onclick=()=>{const id=pendingBookingId;closeNotice();openChangeModal(id)};return modal;
}
function openNotice(bookingId){const b=bookingById(bookingId);if(!b){toastSafe('변경할 예약 정보를 다시 확인해주세요.');return}pendingBookingId=String(b.id||'');ensureNotice().classList.remove('hidden')}
...
function ensureModal(){
  ...
  document.body.appendChild(modal);$('zrChangeModalClose').onclick=closeChangeModal;$('zrChangeFormCancel').onclick=closeChangeModal;$('zrChangeCompleteClose').onclick=closeChangeModal;$('zrChangeReviewEdit').onclick=()=>showStage('form');$('zrChangeReviewOpen').onclick=openReview;$('zrChangeSubmit').onclick=submitChangeRequest;
  $('zrChangeVisitMonth').addEventListener('change',()=>{fillDays('',true);refreshPlayOptions();syncMealOptions()});$('zrChangeVisitDay').addEventListener('change',()=>{refreshPlayOptions();syncMealOptions()});$('zrChangeEntryTime').addEventListener('change',()=>{fillExitOptions('',true);refreshPlayOptions();syncMealOptions()});$('zrChangeExitTime').addEventListener('change',()=>{refreshPlayOptions();syncMealOptions()});$('zrChangeMealMode').addEventListener('change',()=>{syncMealVisibility();fillExitOptions(String($('zrChangeExitTime')?.value||''),true);syncMealOptions()});$('zrChangeMealStart').addEventListener('change',syncMealEnd);$('zrChangeMealDuration').addEventListener('change',syncMealEnd);$('zrChangePlayMode').addEventListener('change',()=>{syncPlayVisibility();refreshPlayOptions()});$('zrChangePlayDuration').addEventListener('change',()=>refreshPlayOptions(String($('zrChangePlayStart')?.value||'')));$('zrChangePlayStart').addEventListener('change',syncPlayEnd);return modal;
}
function showStage(name){$('zrReservationChangeForm')?.classList.toggle('hidden',name!=='form');$('zrReservationChangeReview')?.classList.toggle('hidden',name!=='review');$('zrReservationChangeComplete')?.classList.toggle('hidden',name!=='complete');const card=$('zrReservationChangeModalV1')?.querySelector('.modal-card');setTimeout(()=>{try{card?.scrollTo?.({top:0,behavior:'auto'})}catch{}},0)}
function openChangeModal(bookingId){const booking=bookingById(bookingId);if(!booking){toastSafe('변경할 예약 정보를 다시 확인해주세요.');return}const modal=ensureModal();currentBookingId=String(booking.id||'');pendingBookingId='';loadBookingDefaults(booking);showStage('form');clearInvalid();reviewSnapshot=null;const target=$('zrChangeTargetBooking');if(target)target.textContent=`${booking.date||'-'} · ${booking.entryTime||'--:--'}~${booking.exitTime||'--:--'} · ${booking.orgName||'단체 예약'}`;modal.classList.remove('hidden');try{window.__ZR_MODAL_UX_SYNC_HEADERS?.()}catch{}const card=modal.querySelector('.modal-card');setTimeout(()=>{try{card?.scrollTo?.({top:0,behavior:'auto'})}catch{}},0)}
function closeChangeModal(){$('zrReservationChangeModalV1')?.classList.add('hidden');pendingBookingId='';reviewSnapshot=null;clearInvalid();try{window.__ZR_MODAL_UX_SYNC_HEADERS?.()}catch{}}

function fillMonths(preferred=''){const sel=$('zrChangeVisitMonth');if(!sel)return;const native=$('visitMonth'),nativeOpts=native?.tagName==='SELECT'?[...native.options].filter(o=>/^\d{4}-\d{2}$/.test(String(o.value||''))):[];const rows=nativeOpts.length?nativeOpts.map(o=>({value:String(o.value),text:String(o.textContent||o.value),disabled:!!o.disabled})):(()=>{const out=[],now=new Date();for(let i=0;i<18;i++){const d=new Date(now.getFullYear(),now.getMonth()+i,1),v=`${d.getFullYear()}-${pad2(d.getMonth()+1)}`;out.push({value:v,text:`${d.getFullYear()}년 ${d.getMonth()+1}월`,disabled:false})}return out})();sel.innerHTML='<option value="">월 선택</option>'+rows.map(x=>`<option value="${x.value}"${x.disabled?' disabled':''}>${esc(x.text)}</option>`).join('');if(preferred&&[...sel.options].some(o=>o.value===preferred&&!o.disabled))sel.value=preferred;else sel.value=''}
function fillDays(preferred='',preserve=false){const month=String($('zrChangeVisitMonth')?.value||''),sel=$('zrChangeVisitDay');if(!sel)return;const current=preserve?String(sel.value||''):preferred;if(!/^\d{4}-\d{2}$/.test(month)){sel.innerHTML='<option value="">일 선택</option>';sel.disabled=true;return}const [year,mon]=month.split('-').map(Number),last=new Date(year,mon,0).getDate(),minDate=localTomorrow();const native=$('visitDay'),nativeRows=native?.tagName==='SELECT'?[...native.options].filter(o=>DATE_RE.test(String(o.value||''))&&String(o.value).startsWith(month+'-')&&String(o.value)>=minDate):[];const rows=nativeRows.length?nativeRows.map(o=>{const full=String(o.value);return {day:full.slice(8,10),text:String(o.textContent||`${Number(full.slice(8,10))}일`),disabled:changeDateUnavailable(full,o)}}):(()=>{const out=[];for(let d=1;d<=last;d++){const dd=pad2(d),full=`${month}-${dd}`;if(full<minDate)continue;out.push({day:dd,text:`${d}일`,disabled:changeDateUnavailable(full)})}return out})();sel.innerHTML='<option value="">일 선택</option>'+rows.map(x=>`<option value="${x.day}"${x.disabled?' disabled':''}>${esc(x.text)}</option>`).join('');sel.disabled=false;if(current&&[...sel.options].some(o=>o.value===current&&!o.disabled))sel.value=current;else sel.value=''}
function fillEntryOptions(preferred=''){const sel=$('zrChangeEntryTime');if(!sel)return;const vals=entryValues();sel.innerHTML='<option value="">입장시간 선택</option>'+vals.map(v=>`<option value="${v}">${v}</option>`).join('');if(preferred&&[...sel.options].some(o=>o.value===preferred))sel.value=preferred;else sel.value=''}
function maxVisitMinutes(){return String($('zrChangeMealMode')?.value||'none')==='none'?180:240}
function fillExitOptions(preferred='',cascade=false){const sel=$('zrChangeExitTime');if(!sel)return;const entry=timeToMin(requestEntry()),max=maxVisitMinutes(),vals=[];if(Number.isFinite(entry))for(let m=entry+30;m<=Math.min(entry+max,23*60+30);m+=30)vals.push(minToTime(m));if(preferred&&Number.isFinite(entry)){const pm=timeToMin(preferred);if(Number.isFinite(pm)&&pm>entry&&pm-entry<=max&&!vals.includes(preferred))vals.push(preferred)}vals.sort();sel.innerHTML='<option value="">퇴장시간 선택</option>'+vals.map(v=>`<option value="${v}">${v}</option>`).join('');if(preferred&&vals.includes(preferred))sel.value=preferred;else if(vals.length)sel.value=vals[vals.length-1];else sel.value='';if(cascade){syncMealOptions();refreshPlayOptions()}}
function fillMealStartOptions(preferred=''){const sel=$('zrChangeMealStart');if(!sel)return;const entry=timeToMin(requestEntry()),exit=timeToMin(requestExit()),vals=[];if(Number.isFinite(entry)&&Number.isFinite(exit))for(let m=entry;m<=exit-30;m+=30)vals.push(minToTime(m));sel.innerHTML='<option value="">시작시간 선택</option>'+vals.map(v=>`<option value="${v}">${v}</option>`).join('');if(preferred&&vals.includes(preferred))sel.value=preferred;else sel.value=''}
function syncMealVisibility(){$('zrChangeMealFields')?.classList.toggle('hidden',String($('zrChangeMealMode')?.value||'none')==='none')}
function syncMealOptions(preferredStart){const meal=String($('zrChangeMealMode')?.value||'none');syncMealVisibility();if(meal==='none'){$('zrChangeMealStart').value='';$('zrChangeMealDuration').value='';$('zrChangeMealEnd').value='';return}const current=preferredStart!==undefined?String(preferredStart||''):String($('zrChangeMealStart')?.value||'');fillMealStartOptions(current);syncMealEnd()}
function syncMealEnd(){const start=timeToMin($('zrChangeMealStart')?.value),duration=Number($('zrChangeMealDuration')?.value||0),exit=timeToMin(requestExit());const end=Number.isFinite(start)&&[30,45,60].includes(duration)?start+duration:NaN;$('zrChangeMealEnd').value=Number.isFinite(end)&&end<=exit?minToTime(end):''}
...
function playgroundOccupancies(date,bookingId){return readBookings().filter(row=>{if(!row||sameSource(row,bookingId)||String(row.date||'')!==date)return false;if(['cancelled','rejected'].includes(String(row.status||'')))return false;if(String(row.playUse||'no')!=='yes')return false;const s=timeToMin(row.playStart),e=timeToMin(row.playEnd);return Number.isFinite(s)&&Number.isFinite(e)&&e>s}).map(row=>({start:timeToMin(row.playStart),end:timeToMin(row.playEnd)}))}
function playSlotState(start,duration){const date=requestDate(),booking=selectedBooking(),sm=timeToMin(start),mins=Number(duration||0),entry=timeToMin(requestEntry()),exit=timeToMin(requestExit());if(!DATE_RE.test(date)||!booking||!Number.isFinite(sm)||![30,60].includes(mins)||!Number.isFinite(entry)||!Number.isFinite(exit))return {ok:false,reason:'range'};const end=sm+mins;if(sm<entry||end>exit)return {ok:false,reason:'range'};const conflict=playgroundOccupancies(date,String(booking.id||'')).some(x=>sm<x.end&&end>x.start);return conflict?{ok:false,reason:'full'}:{ok:true,reason:''}}
function syncPlayVisibility(){$('zrChangePlayFields')?.classList.toggle('hidden',String($('zrChangePlayMode')?.value||'no')!=='yes')}
function refreshPlayOptions(preferred){const use=String($('zrChangePlayMode')?.value||'no'),sel=$('zrChangePlayStart');if(!sel)return;syncPlayVisibility();if(use!=='yes'){sel.innerHTML='<option value="">시작시간 선택</option>';$('zrChangePlayEnd').value='';return}const current=preferred!==undefined?String(preferred||''):String(sel.value||''),entry=timeToMin(requestEntry()),exit=timeToMin(requestExit()),duration=Number($('zrChangePlayDuration')?.value||0),date=requestDate();if(!DATE_RE.test(date)||!Number.isFinite(entry)||!Number.isFinite(exit)){sel.innerHTML='<option value="">날짜·입장·퇴장시간을 먼저 선택해주세요</option>';sel.value='';$('zrChangePlayEnd').value='';return}const base=playgroundBaseValues().filter(v=>{const m=timeToMin(v);return Number.isFinite(m)&&m>=entry&&m<exit});let available=0;sel.innerHTML='<option value="">시작시간 선택</option>'+base.map(v=>{const state=[30,60].includes(duration)?playSlotState(v,duration):{ok:true,reason:''};if(state.ok)available++;const suffix=state.reason==='full'?' (마감)':state.reason==='range'?' (이용시간 확인)':'';return `<option value="${v}"${state.ok?'':' disabled'}>${v}${suffix}</option>`}).join('');if(current&&[...sel.options].some(o=>o.value===current&&!o.disabled))sel.value=current;else sel.value='';syncPlayEnd();const help=$('zrChangePlayHelp');if(help)help.textContent=available?'변경 입장시간 이후의 예약 가능한 시간만 선택할 수 있습니다. 마감된 시간은 선택할 수 없습니다.':'현재 조건에서 선택 가능한 놀이터 시간이 없습니다.'}
function syncPlayEnd(){const start=timeToMin($('zrChangePlayStart')?.value),duration=Number($('zrChangePlayDuration')?.value||0),exit=timeToMin(requestExit()),end=Number.isFinite(start)&&[30,60].includes(duration)?start+duration:NaN;$('zrChangePlayEnd').value=Number.isFinite(end)&&end<=exit?minToTime(end):''}

function clearFormValues(){fillMonths('');fillDays('');fillEntryOptions('');$('zrChangeExitTime').innerHTML='<option value="">퇴장시간 선택</option>';$('zrChangeMealMode').value='none';syncMealOptions('');$('zrChangePlayMode').value='no';$('zrChangePlayDuration').value='';refreshPlayOptions('');$('zrChangeNotes').value='';$('zrChangeMealCurrent').textContent='';$('zrChangePlayCurrent').textContent=''}
function loadBookingDefaults(booking){clearInvalid();if(!booking){currentBookingId='';clearFormValues();return}currentBookingId=String(booking.id||'');const date=String(booking.date||''),month=date.slice(0,7),day=date.slice(8,10);fillMonths(month);fillDays(day);fillEntryOptions(String(booking.entryTime||''));const mealType=['lunchbox','cafe','none'].includes(String(booking.mealType||''))?String(booking.mealType):'none';$('zrChangeMealMode').value=mealType;fillExitOptions(String(booking.exitTime||''),false);$('zrChangeMealCurrent').textContent=`현재: ${mealLabel(booking.mealType)}${booking.mealStart?` · ${booking.mealStart}${booking.mealEnd?`~${booking.mealEnd}`:''}`:''}`;syncMealVisibility();if(mealType!=='none'){fillMealStartOptions(String(booking.mealStart||''));const md=rangeDuration(booking.mealStart,booking.mealEnd,[30,45,60],30);$('zrChangeMealDuration').value=String(md);syncMealEnd()}else syncMealOptions('');const playUse=String(booking.playUse||'no')==='yes'?'yes':'no';$('zrChangePlayMode').value=playUse;$('zrChangePlayCurrent').textContent=`현재: ${playLabel(booking.playUse)}${booking.playStart?` · ${booking.playStart}${booking.playEnd?`~${booking.playEnd}`:''}`:''}`;const pd=rangeDuration(booking.playStart,booking.playEnd,[30,60],Number(booking.playDuration)===60?60:30);$('zrChangePlayDuration').value=playUse==='yes'?String(pd):'';syncPlayVisibility();refreshPlayOptions(playUse==='yes'?String(booking.playStart||''):'');$('zrChangeNotes').value=''}

function collectRequest(){
  clearInvalid();const booking=selectedBooking();if(!booking)return failForm('변경할 예약 정보를 다시 불러와주세요.',$('zrChangeTargetBooking'));
  const date=requestDate();if(!DATE_RE.test(date)||date<localTomorrow())return failForm('예약변경날짜를 확인해주세요.','zrChangeVisitMonth');if(isWeekendDate(date))return failForm('주말은 예약변경 날짜로 선택할 수 없습니다.','zrChangeVisitDay');if(isBlockedHoliday(date))return failForm('공휴일 예약이 현재 설정에서 허용되지 않습니다.','zrChangeVisitDay');
  ...
  let mealStart='',mealEnd='',mealDuration=0;if(mealType!=='none'){mealStart=String($('zrChangeMealStart')?.value||'');mealDuration=Number($('zrChangeMealDuration')?.value||0);const ms=timeToMin(mealStart);if(!Number.isFinite(ms))return failForm('식사 시작시간을 선택해주세요.','zrChangeMealStart');if(![30,45,60].includes(mealDuration))return failForm('식사 이용시간을 선택해주세요.','zrChangeMealDuration');const me=ms+mealDuration;if(ms<entryMin||me>exitMin)return failForm('식사시간은 입장·퇴장시간 안에서 선택해주세요.','zrChangeMealStart');mealEnd=minToTime(me)}
  const playUse=String($('zrChangePlayMode')?.value||'no')==='yes'?'yes':'no';let playStart='',playEnd='',playDuration=0;if(playUse==='yes'){playStart=String($('zrChangePlayStart')?.value||'');playDuration=Number($('zrChangePlayDuration')?.value||0);if(!playStart)return failForm('놀이터 시작시간을 선택해주세요.','zrChangePlayStart');if(![30,60].includes(playDuration))return failForm('놀이터 이용시간을 선택해주세요.','zrChangePlayDuration');const state=playSlotState(playStart,playDuration);if(!state.ok)return failForm(state.reason==='full'?'선택한 놀이터 시간은 이미 마감되었습니다. 다른 시간을 선택해주세요.':'놀이터는 변경 입장시간 이후부터 퇴장시간 안에서 선택해주세요.','zrChangePlayStart');playEnd=minToTime(timeToMin(playStart)+playDuration)}
...
function openReview(){const x=collectRequest();if(!x)return;reviewSnapshot=x;const current=x.booking;const mealText=x.mealType==='none'?'식사 안 함':`${mealLabel(x.mealType)} · ${x.mealStart}~${x.mealEnd} (${x.mealDuration}분)`;const playText=x.playUse==='yes'?`이용함 · ${x.playStart}~${x.playEnd} (${x.playDuration}분)`:'이용 안 함';$('zrChangeReviewCard').innerHTML=`<div class="zr-change-review-row"><span>변경 대상 예약</span><b>${esc(`${current.date||'-'} · ${current.entryTime||'--:--'}~${current.exitTime||'--:--'} · ${current.orgName||'단체 예약'}`)}</b></div><div class="zr-change-review-row"><span>변경 요청 일시</span><b>${esc(`${x.date} · ${x.entry}~${x.exit}`)}</b></div><div class="zr-change-review-row"><span>식사</span><b>${esc(mealText)}</b></div><div class="zr-change-review-row"><span>놀이터</span><b>${esc(playText)}</b></div><div class="zr-change-review-row"><span>변경 요청사항</span><b>${esc(x.body||'없음')}</b></div>`;showStage('review')}
function structuredContent(x){const b=x.booking,play=x.playUse==='yes'?`이용함 ${x.playStart}~${x.playEnd} (${x.playDuration}분)`:'이용 안 함',meal=x.mealType==='none'?'식사 안 함':`${mealLabel(x.mealType)} ${x.mealStart}~${x.mealEnd} (${x.mealDuration}분)`,people=Number(b.paidCount||0)+Number(b.chaperoneCount||0);return ['[예약 변경 요청]',`단체명: ${b.orgName||'-'}`,`변경 대상 예약번호: ${b.id||'-'}`,`기존 예약일: ${b.date||'-'}`,`기존 예약시간: ${b.entryTime||'--:--'} ~ ${b.exitTime||'--:--'}`,`예약변경날짜: ${x.date}`,`예약변경시간: ${x.entry} ~ ${x.exit}`,`놀이터: ${play}`,`식사: ${meal}`,`단체 인원: ${people}명`,'',x.body||''].join('\n').trimEnd()}
function waitForChangeBridge(timeout=6000){return new Promise(resolve=>{const started=Date.now();const check=()=>{if(typeof window.zrReservationFirebase?.submitChangeRequest==='function'){resolve(true);return}if(Date.now()-started>=timeout){resolve(false);return}setTimeout(check,100)};check()})}
async function submitChangeRequest(){
  ...
  try{
    if(!await waitForChangeBridge()){showStage('form');failForm('예약 변경 요청 DB 연결을 확인 중입니다. 잠시 후 다시 시도해주세요.',$('zrChangeTargetBooking'));return}
    const booking=bookingById(String(x.booking.id||''));if(!booking){showStage('form');failForm('변경할 예약 정보를 다시 불러와주세요.',$('zrChangeTargetBooking'));return}
    const now=new Date().toISOString(),requestId=`cr_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,people=Number(booking.paidCount||0)+Number(booking.chaperoneCount||0);
    const payload={
      id:requestId,createdAt:now,content:structuredContent({...x,booking}),name:String(booking.managerName||managerValue()),mobile:tel(booking.contact||contactValue()),
      changeRequest:true,changeRequestId:requestId,changeRequestStatus:'pending',changeBookingId:String(booking.id||''),
      changeOldDate:String(booking.date||''),changeOldEntryTime:String(booking.entryTime||''),changeOldExitTime:String(booking.exitTime||''),
      ...
      changeRequestOrgName:String(booking.orgName||''),people,body:x.body
    };
    await window.zrReservationFirebase.submitChangeRequest(payload);
    currentBookingId=String(booking.id||'');try{document.dispatchEvent(new CustomEvent('zr:reservation-change-request-shared',{detail:{bookingId:currentBookingId,requestId}}))}catch{}showStage('complete');
...
function bind(){ensureStyle();ensureChangeSelect();ensureNotice();ensureModal();const setButton=()=>{const btn=$('changeExisting');if(btn){btn.textContent='2. 예약 변경하기';btn.classList.remove('hidden')}};setButton();document.addEventListener('click',e=>{const global=e.target?.closest?.('#changeExisting');if(!global)return;e.preventDefault();e.stopImmediatePropagation();openChangeSelect()},true);document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;if(!$('zrReservationChangeSelectV1')?.classList.contains('hidden')){closeChangeSelect();return}if(!$('zrReservationChangeNoticeV1')?.classList.contains('hidden')){pendingBookingId='';closeNotice();return}if(!$('zrReservationChangeModalV1')?.classList.contains('hidden'))closeChangeModal()});document.addEventListener('zr:reservation-availability-updated',()=>{setButton();if(!$('zrReservationChangeModalV1')?.classList.contains('hidden'))refreshPlayOptions(String($('zrChangePlayStart')?.value||''))});window.addEventListener('storage',e=>{if(e.key!==BOOKING_KEY)return;setButton();if(!$('zrReservationChangeModalV1')?.classList.contains('hidden'))refreshPlayOptions(String($('zrChangePlayStart')?.value||''))});document.addEventListener('zr:customer-runtime-ready',setButton,{once:true});document.addEventListener('zr:reservation-settings-synced',()=>{if(!$('zrReservationChangeModalV1')?.classList.contains('hidden'))fillDays(String($('zrChangeVisitDay')?.value||''),true)})}
