...
need(customer.includes("fetch('./index.html?v=customer-entry-1'"),'customer.html must reuse the frozen reservation runtime instead of copying booking business logic.');
...
need(customer.includes('reservation_firebase_bridge.js?v=1'),'customer.html must preserve the existing shared reservation Firebase bridge.');
...
console.log('OK: dedicated customer.html and admin.html use separate extension runtimes over the unchanged reservation core; customer entry excludes admin tab/patch chains and both entries preserve the shared Firebase/data contracts.');
