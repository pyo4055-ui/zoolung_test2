...
const shim=read('customer_time_guide_guard_v2.js');
const v3=read('customer_guide_transition_v3.js');
try{execFileSync(process.execPath,['--check','customer_time_guide_guard_v2.js'],{stdio:'pipe'})}catch(e){fail(`guard shim syntax: ${e.stderr?.toString()||e.message}`)}
...
for(const needle of ['customer_guide_transition_v3.js?v=3','zrCustomerGuideTransitionV3'])if(!shim.includes(needle))fail(`v2 compatibility loader missing: ${needle}`);
for(const forbidden of ['setTimeout(closeZooGuide,40)','setTimeout(closeZooGuide,120)','exitGuardUntil'])if(shim.includes(forbidden))fail(`old delayed guide guard still active in v2: ${forbidden}`);
