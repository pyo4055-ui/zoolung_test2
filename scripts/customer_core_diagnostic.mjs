...
for(const needle of ['function bookingData','function setStore','const setStore','function addActivity','function bookings','submitBooking','confirmCustomerCancel']){
  let from=0,count=0;
  while(true){
    const at=html.indexOf(needle,from);
    if(at<0)break;
    count++;
    const start=Math.max(0,at-1800),end=Math.min(html.length,at+5200);
