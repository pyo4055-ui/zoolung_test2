...
function counts(){
  ...
  return {
    ...
    pendingInquiry:inquiries.filter(x=>{const t=contentOf(x);return !isPreview(t)&&!hasReply(t)}).length,
...
function observe(){
  ...
  document.addEventListener('zr:inquiry-replies-changed',render);document.addEventListener('zr:preview-visits-changed',render);window.addEventListener('storage',e=>{if([INQUIRY_KEY,'zr_bookings'].includes(e.key||''))render()});
}
